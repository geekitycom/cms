 ...
 *
 * The point of the fixture is that it is the same object each time: whatever
 * the three formats end up printing, they are printing one reading of one
 * post. Where they still differ — RSS names the item by its object id and
 * lists both taxonomies, Atom and JSON Feed name it by its permalink and list
 * tags only, and only RSS summarises a post that wrote no description — the
 * difference is now visible in one file. decision-12 settles those three
 * disagreements; TASK-64 is what changes the bytes.
 */
const ITEM: FeedItem = {
...
/** The same post with nothing optional: no dates, no author, no description. */
const BARE: FeedItem = {
...
describe('an RSS item', () => {
  ...
  it('leaves out what the item does not carry, and still summarises', () => {
    assert.deepEqual(rssItem(BARE), [
      '    <item>',
      '      <title>Bare</title>',
      '      <link>https://example.com/2026/09/bare/</link>',
      '      <guid isPermaLink="false">https://example.com/2026/09/bare/</guid>',
...
describe('an Atom entry', () => {
  it('renders the same item, keyed by its permalink and tagged with its tags', () => {
    assert.deepEqual(atomEntry(ITEM), [
      '  <entry>',
      '    <id>https://example.com/2026/09/hello/</id>',
      '    <title>Hello &amp; &lt;Goodbye&gt;</title>',
  ...
  it('falls back to the epoch for an item with no instants, and summarises nothing', () => {
...
describe('a JSON Feed item', () => {
  ...
  it('leaves out every key the item has nothing for', () => {
